...
将掩码与x进行按位与操作，此时只留下了奇数位的数据，但此时我们依旧不能判断出奇数位是不是都为1，所以要加上一步异或运算，用于判断这个情况。
...
有了，我们可以使用移位乘法，分两个变量，一个用来存储缩小范围后的位模式，一个用来存放此时的位置。最后在往位置上加一（表示1的实际位置）再加一（表示需要一个负权重最高位）即得到最终答案。
