---
title: Writeup for Attack Lab
comments: true
date: 2021-10-31 16:10:00
tags: 
...
还以为把炸弹给拆完，chapter3就完结撒花了，结果没想到还有一个attack lab

这个实验是给我们体验缓冲区溢出攻击的，算是有点意思

攻击的两个方法分别是：**Code Injection** 和 **Return-Orient Programming**，即代码注入和面向返回编程

记得，再开始之前必须，必须，必须得看一看本实验的Writeup！实验怎么做以及实验的相关信息都在里头
...
由自己写的汇编构造字节码的时候，可以这么操作，先编译为目标文件后进行反汇编，下面为一个例子
...
在调用`getbuf()`的时候，我们就要输入我们构造的字符串，使得程序跳到touch1 2 3这几个函数中去。
...
PASS: Would have posted the following:
        ...
        course  15213-f15
        lab     attacklab
        result  1:PASS:0xffffffff:ctarget:1:11 45 14 19 19 81 00 00 11 45 14 19 19 81 00 00 11 45 14 19 19 81 00 00 11 45 14 19 19 81 00 00 11 45 14 19 19 81 00 00 C0 17 40 00 00 00 00 00
```
...
所以最后构造出来的字节码如下，注意，汇编字节码的不需要按大小端序进行排布
...
思考一下，直接把`%rsp`给拉下来`0x28+0x8`即`0x30`就行了
...
首先我们无法确定栈的地址，即使能确定，但由于栈的内存区域被标记为不可执行的，所以一旦执行位于栈上的代码就会出现`segment fault`
...
但仔细一看，`c3`前的片段`48 89 c7`其实是`movq %rax, %rdi`的字节码，要想执行这段代码，我们只需要通过`ret`命令将PC给设定为对应的代码的地址即可。这样，一段汇编就这样被构造出来了
...
这个好像writeup没建议要做，因为只有5分，却要构造8个gadget，他还说前面四个phase你对解决了就已经95分了，言外之意就是：~~除非你是卷王，否则就不要碰了~~
...
难点是，如何在栈随机化的情况下得到字符串的起始地址。随机化的起始地址，给我们的信息就是：绝对地址已经不可能了
...
那我们先把farm里头的字节码序列都分析一波呗，看看能得出什么有用的序列

结合phase4的分析，最后得出如下的语句
